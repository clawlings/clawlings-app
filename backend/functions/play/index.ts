...
Deno.serve(async (req) => {
  ...
  if (last) {
    ...
    if (elapsed < 300000) {
      ...
      return errorResponse(`You played with your pet ${agoMin}m ago. Wait ${waitMin} more minute${waitMin > 1 ? "s" : ""}.`, 429, req);
    }
  }

  const { data: updated, error: err } = await supabase
    .from("pets")
    .update({ happiness: Math.min(pet.happiness + 15, 100) })
    .eq("id", pet_id)
    .select("*")
    .single();
  ...
  const { error: logErr } = await supabase.from("interactions").insert({ pet_id, action: "play" });
